...
%
% this is a wrapper for "check_fixedpt"; 
% also calls graph2net.m and check_stability.m.
%
% created Mar 23, 2016
% modified on June 3, 2017 to compute sum sgn det(I-W_sig) formula
% modified on Oct 29, 2018 to allow for a vector of theta inputs
% modified on June 11, 2020 to check fixed points grouped by the
% size of their supports
% modified on June 17, 2020 to include a print_flag of whether or not
% to print out the fixed point supports
% last modified Aug 23, 2023 for CTLN Basic 2.0 (minor cosmetic changes)
...
for k=1:n
    ...
    for i=1:size(sets,1)
        ...
        if TF
            ...
            if print_flag
                fprintf(['index = ' int2str(sgn(j)) ', supp = ' int2str(sig) '\n']);
            end
            if nargout > 2
...
if print_flag
    fprintf(['number of fixed points = ' int2str(length(sgn)) '\n']);
    fprintf(['parity check: sum of sgn(I-W_sigma) = ' int2str(sum(sgn)) '\n']);
end